...
/proc/ManifestToHtml()
	var/html = ""
	if(data_core)
		html = data_core.get_manifest(FALSE,TRUE)
	else
...
/datum/tgs_chat_command/discordping/Run(datum/tgs_chat_user/sender, params)
	...
	var/DBQuery/query = dbcon.NewQuery("SELECT discord_id FROM erro_player WHERE ckey = :t_ckey",list("t_ckey" = key_to_find))
	query.Execute()
...
/datum/tgs_chat_command/getkey/Run(datum/tgs_chat_user/sender, params)
	...
	var/DBQuery/query = dbcon.NewQuery("SELECT ckey FROM erro_player WHERE discord_id = :t_discord", list("t_discord"=params))
	query.Execute()
